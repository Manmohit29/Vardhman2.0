...
def post_now_production(production_data):
    ...
    if SEND_DATA:
        try:
            req = requests.post(f"{HOST_FP}/now_production", json=payload, headers=HEADERS, timeout=5)
...
def post_now_stoppage(stoppage_data):
    ...
    if SEND_DATA:
        try:
            req = requests.post(f"{HOST_FP}/now_stoppage", json=payload, headers=HEADERS, timeout=5)
...
def send_po_data(data):
    ...
    if SEND_DATA:
        try:
            req = requests.post(f"{HOST_FP}/create_po_data", json=data, headers=HEADERS, timeout=5)
